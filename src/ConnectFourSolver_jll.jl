...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import ConnectFourSolver_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("5ab91c8a-5c16-538d-97cb-84915cefd4b0"))
platforms = [Pkg.Artifacts.unpack_platform(e, "ConnectFourSolver", artifacts_toml) for e in artifacts["ConnectFourSolver"]]
...
if best_platform === nothing
    @debug("Unable to load ConnectFourSolver; unsupported platform $(triplet(platform_key_abi()))")
else
